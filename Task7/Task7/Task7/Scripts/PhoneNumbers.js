...
app.controller('PhoneModalCtrl', function ($scope, $uibModalInstance, phoneNumber, Devices,ExistingPhones) {
    ...
    $scope.save = function () {
        ...
        if (!$scope.ModalPhoneNumber.Number || $scope.ModalPhoneNumber.Number.length !== 8) {
            $scope.errors.number = "Phone number must be exactly 8 digits.";
        }

        if (!$scope.ModalPhoneNumber.DeviceID) {
            $scope.errors.device = "Please select a device.";
        }

        var exists = ExistingPhones.some(function (p) {
            return p.Number === $scope.ModalPhoneNumber.Number && p.ID !== $scope.ModalPhoneNumber.ID;
        });

        if (exists) {
            $scope.errors.number = "Phone number already exists.";
            return;
        }

        // If any errors exist, do not proceed
        if (Object.keys($scope.errors).length > 0) return;
