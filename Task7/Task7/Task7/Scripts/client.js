...
app.controller('ClientCtrl', function ($scope, $http, $uibModal) {
    ...
    $scope.deleteClient = function (client) {
        $uibModal.open({
            ...
            $scope.loadClients(); // refresh client list if delete successful
        });
    };
...
app.controller('ClientModalCtrl', function ($scope, $uibModalInstance, client, clients) {
    ...
    $scope.save = function () {
        ...
        if (!client.Name || client.Name.trim() === "") {
            $scope.errors.name = "Client name cannot be empty.";
        }

        if (client.Type === undefined || client.Type === null || client.Type === "") {
            $scope.errors.type = "Please select a client type.";
        }
        ...
        if (typeInt === 0) {
            if (!client.BirthDate) {
                $scope.errors.birthDate = "BirthDate is required for Individual clients.";
            } else {
                ...
                if (actualAge < 18) {
                    $scope.errors.birthDate = "Client must be at least 18 years old.";
                }
            }
        }
        ...
        if (duplicate) {
            $scope.errors.name = "Duplicate client name for the same type is not allowed.";
        }
...
app.controller('DeleteClientModalCtrl', function ($scope, $uibModalInstance, $http, client) {
    $scope.client = client;
    $scope.errorMessage = '';
    ...
    $scope.confirm = function () {
        $http.delete('/api/clients/' + client.ID)
            ...
            .catch(function (error) {
                $scope.errorMessage = error.data?.Message || "Delete failed due to server error.";
            });
    };
